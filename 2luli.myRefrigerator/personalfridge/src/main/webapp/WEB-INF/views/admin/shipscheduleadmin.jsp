		        ...
		        <c:forEach var="ship" items="${shipList}">
		            <div class="card">
		                ...
		                <div class="card-info" style="margin-bottom:-2px;">
		                    ...
		                    <a><strong>사용자:</strong> ${ship.user_id}<strong style="margin-left:5px;">송장 번호:</strong>${ship.ship_code}</a>
		                    <!-- 필요한 경우 추가적인 정보 표시 -->
		                </div>
		            </div>
		        </c:forEach>
 ...
 // 모달이 열려 있는지 여부를 추적하는 변수
    ...
    function addDeleveryAndShipInfo() {
        $('.card-info').each(function() {
            var ButtonsHTML = `
                <div class="cardbtn">
            		<button class="btn btn-primary delete" >배송 삭제</button>
                    <button class="btn btn-danger shipschedule" style="margin:0 5px;" >배송 현황</button>
                </div>`;
    ...
    //배송 현황 보여주기
     $(document).ready(function() {
    $(document).on('click', '.shipschedule', function() {
        var shipCode = $(this).closest('.card').find('.header-text').text();
        $.ajax({
            type: "GET",
            url: "/admin/getshipschduleAdmin",
                ...
                if (response.groupedShipList && Object.keys(response.groupedShipList).length > 0) {
                	 var shipCode = Object.keys(response.groupedShipList)[0]; // 첫 번째 송장 번호 키 가져오기
             	     ...
             	     for (var i = 0; i < group.length; i++) {
                        ...
                        '<div><strong>위치:</strong> ' + group[i].now_location + '</div>' +
    ...
    $(document).on('click', '.delete', function() {
        ...
        $(document).on('click', '#confirmShipButton', function() {
            ...
            $.ajax({
                type: "POST",
                url: "/admin/deleteShipByShipCode",
